...
- 连续出现 deflect / reject / 边界信号时,不要为了礼貌降成"再观察";
  宿主需要知道"不合在哪里",不是被安慰"还可以再看看"。
...
- "来电"必须能被原文复核:如果 highlights 写不出"双方各自哪句话把关系往前推了",
  不准判来电
- 写"来电"卡片时要克制:只说原文里能看到的动作、选择、边界差异,不要用
  "翻版 / 命中 / 灵魂同频 / 真的接住 / 对上电" 这类替用户加戏的词。
- 如果"来电"主要靠单一长话题成立,可以判,但 risks 里要提醒"还没换场景确认",
  不要把它写成全面合拍。
...
- highlights[].text:"@对方 在认知科学那段没有只顺着夸,而是接着问你怎么处理不确定性,你也补了自己的边界,这段是真往里聊了一层。"
- risks[].text:"这场主要还停在一个话题里,换到生活节奏或冲突处理时还能不能这么顺,真人聊时要再看一下。"
"""
...
def _visible_mismatch_message(messages: list[AgentChatMessage]) -> AgentChatMessage | None:
    for msg in messages:
        text = str(msg.utterance or "")
        private = msg.private_signals or {}
        if str(msg.intent or "") == "reject":
            return msg
...
def _strong_mismatch_reason(
    ...
    user_ids: set[int],
) -> tuple[str, AgentChatMessage | None] | None:
    """Return a reason when "不合" is visibly supported."""
    if chat.end_reason and chat.end_reason.startswith("boundary_hit_"):
        return "这场触到了明确边界。", None

    visible = _visible_mismatch_message(messages)
    if visible is not None:
        return "原文里已经有明确降温或不接的表达。", visible
...
def _prepend_guard_highlight(data: dict, reason: str, message: AgentChatMessage | None) -> None:
    highlights = data.get("highlights")
    if not isinstance(highlights, list):
        highlights = []
    highlights.insert(0, {
        "text": f"我会把这场判成不合，{reason}",
        "evidence_utterance_id": message.id if message is not None else None,
    })
    ...
    *,
    ...
    for host_user_id in [match.user_a_id, match.user_b_id]:
        ...
        if verdict != "来电" and recommended == "开真人聊天":
            recommended = "再派一次"
        mismatch = _strong_mismatch_reason(
            chat,
            messages,
            {match.user_a_id, match.user_b_id},
        )
        if mismatch is not None:
            reason, evidence_message = mismatch
            if verdict != "不合":
                _prepend_guard_highlight(data, reason, evidence_message)
            verdict = "不合"
            recommended = "跟我聊聊调方向"
        if verdict == "来电":
